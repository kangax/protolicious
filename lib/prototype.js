...
if (!Array.prototype.reduce) Array.prototype.reduce = function(callback, initial) {
  var array = this, _valueProvided = false;

  if (Object.isUndefined(initial))
    initial = this.shift();
  else _valueProvided = true;

  this.each(function(value, index) {
    initial = callback.call(callback, initial, value, _valueProvided ? index : index + 1, array);
  });
  return initial;
}
...
Element.Methods = {
  ...
  getDimensions: function(element) {
    ...
    if (display != 'none' && display != null) { // Safari bug
      // Mozilla includes margin offset in table offsetWidth
      if (element.tagName.toLowerCase() == 'table') {
        var originalLeft = element.style.marginLeft,
          originalRight = element.style.marginRight, width;
      ...
      return {width: width || element.offsetWidth, height: element.offsetHeight};
    }
  ...
  viewportOffset: function(forElement) {
    ...
    do {
      ...
      // Firefox returns negative offset when document.body border is > 0
      if (parent == document.body) {
